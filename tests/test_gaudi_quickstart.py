#!/usr/bin/env python3
"""
Tests for Gaudi Quickstart Documentation (Stage 2.4)
...
def quickstart_dir(project_root):
    """Path to Gaudi quickstart directory"""
    return project_root / "docs" / "quickstarts" / "gaudi-hello-world"
...
class TestREADMEContent:
    ...
    def test_prerequisites_mention_gaudi(self, quickstart_dir):
        """Prerequisites should mention Gaudi requirements"""
        readme = quickstart_dir / "README.md"
        ...
        # Should mention Gaudi-specific requirements
        assert 'gaudi' in content, "Prerequisites should mention Gaudi"
        assert 'habana' in content or 'hpu' in content, \
            "Should mention Habana or HPU"
    ...
    def test_readme_references_manifest_directory(self, quickstart_dir):
        ...
        assert re.search(r'deploy/gaudi-inference', content), \
            "README should reference deploy/gaudi-inference directory"
    ...
    def test_readme_shows_expected_output(self, quickstart_dir):
        ...
        has_output_examples = (
            ...
            'returns:' in content.lower() or
            'expected output' in content.lower()
        )

        assert has_output_examples, \
            "README should show expected output for key commands"

    def test_readme_explains_cpu_vs_gaudi(self, quickstart_dir):
        """README should explain when to use Gaudi vs CPU"""
        readme = quickstart_dir / "README.md"
        ...
        # Should compare CPU and Gaudi
        has_comparison = (
            'cpu vs' in content or
            'gaudi vs' in content or
            'when to use' in content or
            'cpu path' in content
        )
    ...
    def test_readme_mentions_gaudi_resources(self, quickstart_dir):
        """README should mention habana.ai/gaudi resources"""
        readme = quickstart_dir / "README.md"
        ...
        # Should mention the Gaudi resource type
        assert 'habana.ai/gaudi' in content, \
            "README should mention habana.ai/gaudi resource type"
...
class TestReferencedFiles:
    ...
    def test_manifest_files_exist(self, quickstart_dir, project_root):
        ...
        manifest_dir = project_root / "deploy" / "gaudi-inference"
...
class TestTestScript:
    ...
    def test_test_script_checks_gaudi_assignment(self, quickstart_dir):
        """test.sh should verify Gaudi GPU assignment"""
        test_script = quickstart_dir / "test.sh"
        ...
        # Should check for Gaudi resource
        has_gaudi_check = (
            'habana.ai/gaudi' in content or
            'gaudi' in content.lower() or
            'habana' in content.lower()
        )

        assert has_gaudi_check, "test.sh should verify Gaudi GPU assignment"
...
def test_validation_matrix_gaudi_quickstart(project_root):
    """Track validation matrix results for Gaudi quickstart"""
